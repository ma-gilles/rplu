...
struct di_aligned_allocator {
    ...
    template <typename U>
    void construct(U *p) const noexcept(noexcept(::new (const_cast<void *>(static_cast<const void *>(p))) U))
    {
    ...
    void construct(U *p, Args &&... args) const
        noexcept(noexcept(::new (const_cast<void *>(static_cast<const void *>(p))) U(std::forward<Args>(args)...)))
    {
