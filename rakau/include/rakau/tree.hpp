...
struct morton_encoder<3, std::uint64_t> {
    ...
    {
        using val_t = it_value_type<It>;
        static_assert(std::is_integral_v<val_t> && sizeof(val_t) == sizeof(std::uint64_t));
        const auto x = *it;
        const auto y = *(it + 1);
        const auto z = *(it + 2);
        assert(x < (1ul << 21));
...
// Convenience wrappers so platforms where unsigned long != std::uint64_t
// still hit the Morton specialisations.
template <std::size_t NDim>
struct morton_encoder<NDim, unsigned long> {
    template <typename It>
    unsigned long operator()(It it) const
    {
        static_assert(sizeof(unsigned long) == sizeof(std::uint64_t),
                      "unsigned long expected to be 64-bit here");
        return static_cast<unsigned long>(morton_encoder<NDim, std::uint64_t>{}(it));
    }
};

template <std::size_t NDim>
struct morton_decoder<NDim, unsigned long> {
    template <typename It>
    void operator()(It it, unsigned long code) const
    {
        static_assert(sizeof(unsigned long) == sizeof(std::uint64_t),
                      "unsigned long expected to be 64-bit here");
        morton_decoder<NDim, std::uint64_t>{}(it, static_cast<std::uint64_t>(code));
    }
};
            ...
            if constexpr (Q == 0u) {
                ...
                for (size_type i1 = 0; i1 < tgt_size; i1 += batch_size) {
                    ...
                    for (size_type i2 = i1 + 1u; i2 < tgt_size; ++i2) {
                        ...
                        // Compute the potentials on 1 and 2 using only source mass.
                        const auto pot_1 = mvec2 / (NDim == 2u ? dist2 : xsimd_sqrt(dist2));
                        const auto pot_2 = mvec1 / (NDim == 2u ? dist2 : xsimd_sqrt(dist2));
                        res_vec += pot_1;
                        (batch_type(res + i2, xsimd::unaligned_mode{}) + pot_2).store_unaligned(res + i2);
                    }
                ...
                for (size_type i1 = 0; i1 < tgt_size; i1 += batch_size) {
                    ...
                    for (size_type i2 = i1 + 1u; i2 < tgt_size; ++i2) {
                        ...
                        // Compute the potentials using only source mass.
                        const auto pot_1 = mvec2 / (NDim == 2u ? dist2 : xsimd_sqrt(dist2));
                        const auto pot_2 = mvec1 / (NDim == 2u ? dist2 : xsimd_sqrt(dist2));
                        // Add to the accumulators for 1 the accelerations due to the batch 2,
                        // and accumulate the potentials.
                        res_x_vec1 = xsimd_fma(diff_x, m2_dist3, res_x_vec1);
                        res_y_vec1 = xsimd_fma(diff_y, m2_dist3, res_y_vec1);
                        res_z_vec1 = xsimd_fma(diff_z, m2_dist3, res_z_vec1);
                        res_pot_vec += pot_1;
                        // Add *directly into the result buffer* the acceleration on 2 due to 1,
                        // and accumulate the potential.
                        xsimd_fnma(diff_x, m1_dist3, batch_type(res_x + i2, xsimd::unaligned_mode{}))
                            .store_unaligned(res_x + i2);
                        xsimd_fnma(diff_y, m1_dist3, batch_type(res_y + i2, xsimd::unaligned_mode{}))
                            .store_unaligned(res_y + i2);
                        xsimd_fnma(diff_z, m1_dist3, batch_type(res_z + i2, xsimd::unaligned_mode{}))
                            .store_unaligned(res_z + i2);
                        (batch_type(res_pot + i2, xsimd::unaligned_mode{}) + pot_2).store_unaligned(res_pot + i2);
                    }
            ...
            for (size_type i1 = 0; i1 < tgt_size; ++i1) {
                ...
                for (size_type i2 = i1 + 1u; i2 < tgt_size; ++i2) {
                    ...
                    if constexpr (Q == 1u || Q == 2u) {
                        ...
                        // Potential on i1 due to source i2 uses only source mass.
                        const auto pot_i1 = m2 / (NDim == 2u ? dist2 : dist);
                        const auto pot_i2 = m1 / (NDim == 2u ? dist2 : dist);
                        // Accumulate positively (kernel sum).
                        a1[pot_idx] += pot_i1;
                        res_ptrs[pot_idx][i2] += pot_i2;
                    }
                }
            ...
            for (size_type i1 = 0; i1 < tgt_size; ++i1) {
                ...
                for (size_type i2 = src_begin; i2 < src_end; ++i2) {
                    ...
                    if constexpr (Q == 1u || Q == 2u) {
                        ...
                        // 0 if only the potentials are requested, NDim otherwise.
                        constexpr auto pot_idx = static_cast<std::size_t>(Q == 1u ? 0u : NDim);
                        const auto pot_denom = NDim == 2u ? dist2 : dist;
                        res_ptrs[pot_idx][i1] = fma_wrap(F(1), m2 / pot_denom, res_ptrs[pot_idx][i1]);
                    }
                }
            }
        }
        ...
        if constexpr (simd_enabled && NDim == 3u) {
            ...
            if constexpr (Q == 0u) {
                ...
                for (size_type i = 0; i < tgt_size; i += batch_size) {
                    ...
                    const auto m_src_dist_vec = use_fast_inv_sqrt<batch_type>
                                                    ...
                                                    : m_src_vec / batch_type(tmp_dist + i, xsimd::aligned_mode{});
                    // Compute and accumulate the potential (source-weight only).
                    (batch_type(res + i, xsimd::aligned_mode{}) + m_src_dist_vec).store_aligned(res + i);
                }
            } else {
                // Q == 2, accelerations and potentials.
                //
                ...
                for (size_type i = 0; i < tgt_size; i += batch_size) {
                    ...
                    xsimd_fma(zdiff, m_src_dist3_vec, batch_type(res_z + i, xsimd::aligned_mode{}))
                        .store_aligned(res_z + i);
                    // Compute and accumulate the potential (source-weight only).
                    (batch_type(res_pot + i, xsimd::aligned_mode{}) + m_src_dist_vec).store_aligned(res_pot + i);
                }
            }
        } else {
            ...
            for (size_type i = 0; i < tgt_size; ++i) {
                ...
                if constexpr (Q == 1u || Q == 2u) {
                    ...
                    const auto dist = tmp_ptrs[dist_idx][i];
                    const auto pot_term = m_src / (NDim == 2u ? dist * dist : dist);
                    res_ptrs[pot_idx][i] += pot_term;
                }
            }
        }
    }
    ...
    auto exact_acc_pot_impl(size_type orig_idx, F G, F eps) const
    {
        ...
        for (size_type i = 0; i < size; ++i) {
            ...
            if constexpr (Q == 1u || Q == 2u) {
                ...
                // 0 if only the potentials are requested, NDim otherwise.
                constexpr auto pot_idx = static_cast<std::size_t>(Q == 1u ? 0u : NDim);
                const auto pot_term = NDim == 2u ? Gmi_dist * inv_dist : Gmi_dist;
                retval[pot_idx] += pot_term;
            }
        }
        return retval;
    }
