...
{
    // Newer CUDA toolchains take a cudaMemLocation for the device selector.
#if defined(__CUDACC_VER_MAJOR__) && __CUDACC_VER_MAJOR__ >= 12
    ::cudaMemLocation loc{};
    loc.type = ::cudaMemLocationTypeDevice;
    loc.id = device;
    const auto ret = ::cudaMemAdvise(ptr, count, advice, loc);
#else
    ...
    for (auto src_idx = 0; src_idx < tree_size;) {
        ...
        if (__all_sync(unsigned(-1), s_p_code != src_code && mac_lh < dist2)) {
            ...
            if (Q == 1u || Q == 2u) {
                ...
                // Source-weighted 1/r^2 potential in 2D.
                res_array[pot_idx] += props[NDim] / dist2;
            }
            ...
            if (!n_children_src) {
                ...
                for (auto i = src_begin; i < src_end; ++i) {
                    ...
                    if (Q == 1u || Q == 2u) {
                        ...
                        // 0 if only the potentials are requested, NDim otherwise.
                        constexpr auto pot_idx = static_cast<std::size_t>(Q == 1u ? 0u : NDim);
                        // Source-weighted 1/r^2 potential (skip self term).
                        res_array[pot_idx] += !is_tgt_particle * (m_i / dist2);
                    }
                }
            }
